...
import NavLink from '../ComponentsTwo/NavLink.vue';
...
<template>
    ...
    <header class="bg-slate-800 text-white">
        <nav class="p-6 mx-auto max-w-screen-lg">
            <NavLink routeName="home" componentName="HomeTwo">Home</NavLink>
            <div class="flex items-center space-x-6">
                <!-- Auth -->
                <div v-if="user" class="relative flex items-center gap-4">
                    <div class="flex items-center gap-2 px-3 py-1 rounded-lg hover:bg-slate-700 cursor-pointer"
                    ...
                    class="hover:bg-slate-700 w-6 h-6 grid place-items-center rounded-full hover:outline outline-1
                    outline-white">
                        <i class="fa-solid fa-lock"></i>
                    </Link>
                    <!-- User dropdown menu -->
                     <div class="absolute z-50 top-16 right-0 bg-slate-800 text-white rounded-lg border-slate-300 border overflow-hidden w-40" v-show="show" @click="show=false">
                ...
                <div v-else class="space-x-6">
                    <NavLink routeName="login" componentName="Auth/LoginTwo">Login</NavLink>
                    <NavLink routeName="register" componentName="Auth/RegisterTwo">Register</NavLink>
                </div>
                <button
                @click="switchTheme"
