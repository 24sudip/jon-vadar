...
<template>
    ...
    <Container class="flex gap-4">
        <div class="w-1/4 rounded-md overflow-hidden">
            <img :src="listing.image ? `/storage/${listing.image}` : '/storage/images/listing/default-listing.jpg'" class="w-full h-full object-cover object-center">
        </div>
        <div class="w-3/4">
            <!-- Listing Info -->
            <div class="mb-6">
                <div class="flex items-end justify-between mb-2">
                    ...
                    <div v-if="canModify" class="pl-4 flex items-center gap-4">
                        <Link class="bg-green-500 rounded-md text-white px-6 py-2 hover:outline outline-green-500 outline-offset-2" :href="route('listing.edit', listing.id)">
                            Edit
                        </Link>
                        <button class="bg-red-500 rounded-md text-white px-6 py-2 hover:outline outline-red-500
                        outline-offset-2" @click="deleteListing" type="button">
                            Delete
                        </button>
                    </div>
                </div>
            ...
            <div v-if="listing.tags" class="mb-6">
                ...
                <div class="flex items-center gap-3">
                    <div v-for="tag in listing.tags.split(',')" :key="tag">
                        <Link class="bg-slate-500 text-white px-2 py-1 rounded-full hover:bg-slate-700
                        dark:hover:bg-slate-900" :href="route('home', { tag })">{{ tag }}</Link>
                    </div>
                </div>
            </div>
        </div>
