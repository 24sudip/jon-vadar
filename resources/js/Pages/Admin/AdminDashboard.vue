...
import RoleSelect from '../../ComponentsTwo/RoleSelect.vue';
...
<template>
    ...
    <div class="flex items-end justify-between mb-4">
        ...
        <div class="flex items-center gap-1 text-xs hover:bg-slate-300 dark:hover:bg-slate-800 px-2 py-1 rounded-md">
            <input type="checkbox" id="toggleRole" class="rounded-md border-1 outline-0 text-indigo-500 ring-indigo-500 border-slate-700 cursor-pointer" @input="toggleRole" :checked="params.user_role">
        ...
        <tbody class="divide-y divide-slate-300 divide-dashed">
            <tr v-for="user in users.data" :key="user.id">
                ...
                <td class="w-1/6 py-5 px-3 text-right">
                    <Link class="fa-solid fa-up-right-from-square px-3 text-indigo-400" :href="route('user.show', user.id)">
                    </Link>
                </td>
            </tr>
        </tbody>
    ...
    <div class="mt-6">
        <PaginationLinkTwo :paginator="users" />
    </div>
</template>
